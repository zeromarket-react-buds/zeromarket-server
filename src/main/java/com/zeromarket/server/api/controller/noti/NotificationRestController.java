...
public class NotificationRestController {
    ...
    @ResponseStatus(org.springframework.http.HttpStatus.NO_CONTENT)
    public ResponseEntity<Integer> markChatRoomAsRead(
        @Parameter(hidden = true)
        @AuthenticationPrincipal CustomUserDetails userDetails,
        ...
        Long memberId = userDetails.getMemberId();
        int result = notificationService.markChatRoomAsRead(memberId, chatRoomId);
        return ResponseEntity.ok(result);
    }

    @PatchMapping("/notifications/read/by-ref")
    public ResponseEntity<Integer> markReadByRef(
        @AuthenticationPrincipal CustomUserDetails userDetails,
        @RequestBody NotificationDto req
    ) {
        req.setMemberId(userDetails.getMemberId());
        int updated = notificationService.markReadByRef(req);
